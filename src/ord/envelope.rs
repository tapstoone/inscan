use {
  ...
  bitcoin::{
    ...
    TxIn, TxOut, Txid, OutPoint, Sequence
  },
  std::iter::Peekable,
...
impl From<RawEnvelope> for ParsedEnvelope {
  fn from(envelope: RawEnvelope) -> Self { 
    ...
    let body = envelope
      ...
      .iter()
      .enumerate()
      .position(|(i, push)| i % 2 == 0 && push.is_empty()); //TODO: it's not satisified for atom , atom contain only 3 item, but here need 4. so the body will be NOne
...
impl RawEnvelope {
  ...
  fn from_tapscript(tapscript: &Script, input: usize, protocol_id: &[u8]) -> Result<Vec<Self>> {
    ...
    let mut instructions = tapscript.instructions().peekable(); //convert the tapscript into iterable data
    // println!("\nfrom_tapscript->instructions: {:?}", instructions);
  ...
  //       payload: [
  //           [1], 
  //           [116, 101, 120, 116, 47, 112, 108, 97, 105, 110, 59, 99, 104, 97, 114, 115, 101, 116, 61, 117, 116, 102, 45, 56], 
  //           [], 
  //           [123, 34, 112, 34, 58, 34, 98, 114, 99, 45, 50, 48, 34, 44, 34, 111, 112, 34, 58, 34, 116, 114, 97, 110, 115, 102, 101, 114, 34, 44, 34, 116, 105, 99, 107, 34, 58, 34, 116, 114, 97, 99, 34, 44, 34, 97, 109, 116, 34, 58, 34, 57, 56, 34, 125]
  //       ], 
  //       pushnum: false, 
  //       stutter: false 
  //   }
  ...
  fn from_instructions(
    ...
    loop {
      // println!("\nfrom_instructions->instructions-2: {:?}", instructions);
      match instructions.next().transpose()? {
        ...
        Some(PushBytes(push)) => { // how to check this is PushBytes, but not others? is it because the OP_IF already filtered before?
